...
**Moltbot** 是一个运行在您自有设备上的*个人 AI 助手*。
它可以在您已经使用的渠道上与您对话（WhatsApp、Telegram、Slack、Discord、Google Chat、Signal、iMessage、Microsoft Teams、WebChat），以及扩展渠道如 BlueBubbles、Matrix、Zalo 和 Zalo Personal。它可以在 macOS/iOS/Android 上进行语音通话和聆听，还可以渲染一个由您控制的实时画布。网关只是控制平面——产品本身才是助手。

如果您想要一个感觉本地化、快速且始终在线的个人单用户助手，这就是您要找的。
...
推荐设置：运行引导向导（`moltbot onboard`）。它会引导您完成网关、工作区、频道和技能的设置。CLI 向导是推荐的方式，适用于 **macOS、Linux 和 Windows（通过 WSL2；强烈推荐）**。
支持 npm、pnpm 或 bun。
新安装？从此处开始：[入门指南](https://docs.molt.bot/start/getting-started)

**订阅（OAuth）：**
- **[Anthropic](https://www.anthropic.com/)**（Claude Pro/Max）
- **[OpenAI](https://openai.com/)**（ChatGPT/Codex）

模型说明：虽然支持任何模型，但我强烈推荐 **Anthropic Pro/Max（100/200）+ Opus 4.5**，因为它在长上下文和更好的提示注入抵抗方面表现出色。请参阅[引导](https://docs.molt.bot/start/onboarding)。

## 模型（选择 + 身份验证）

- 模型配置 + CLI：[模型](https://docs.molt.bot/concepts/models)
- 身份验证配置文件轮换（OAuth vs API 密钥）+ 故障转移：[模型故障转移](https://docs.molt.bot/concepts/model-failover)
...
传统说明：`clawdbot` 仍作为兼容层提供。

## 快速开始（摘要）

运行时：**Node ≥22**。

完整的初学者指南（身份验证、配对、频道）：[入门指南](https://docs.molt.bot/start/getting-started)
...
# 与助手对话（可选地传递回任何已连接的频道：WhatsApp/Telegram/Slack/Discord/Google Chat/Signal/iMessage/BlueBubbles/Microsoft Teams/Matrix/Zalo/Zalo Personal/WebChat）
...
- **stable**：标记的发布版本（`vYYYY.M.D` 或 `vYYYY.M.D-<patch>`），npm 发行标签 `latest`。
- **beta**：预发布标签（`vYYYY.M.D-beta.N`），npm 发行标签 `beta`（macOS 应用可能缺失）。
- **dev**：主分支的动态更新，npm 发行标签 `dev`（当发布时）。
...
对于从源码构建，建议使用 `pnpm`。Bun 可选用于直接运行 TypeScript。
...
注意：`pnpm moltbot ...` 直接运行 TypeScript（通过 `tsx`）。`pnpm build` 生成 `dist/` 用于通过 Node 或打包的 `moltbot` 二进制文件运行。
...
- 使用以下命令批准：`moltbot pairing approve <channel> <code>`（然后发送者被添加到本地允许列表存储中）。
- 公共入站 DM 需要明确的加入：设置 `dmPolicy="open"` 并在频道允许列表中包含 `"*"`（`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`）。

运行 `moltbot doctor` 以发现风险/配置错误的 DM 策略。

## 主要特性

- **[本地优先网关](https://docs.molt.bot/gateway)** — 用于会话、频道、工具和事件的单一控制平面。
...
- **[多智能体路由](https://docs.molt.bot/gateway/configuration)** — 将入站频道/账户/对等方路由到隔离的智能体（工作区 + 每个智能体的会话）。
- **[语音唤醒](https://docs.molt.bot/nodes/voicewake) + [对话模式](https://docs.molt.bot/nodes/talk)** — macOS/iOS/Android 上始终在线的语音，支持 ElevenLabs。
- **[实时画布](https://docs.molt.bot/platforms/mac/canvas)** — 智能体驱动的可视化工作区，支持 [A2UI](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui)。
- **[一流工具](https://docs.molt.bot/tools)** — 浏览器、画布、节点、定时任务、会话和 Discord/Slack 操作。
- **[配套应用](https://docs.molt.bot/platforms/macos)** — macOS 菜单栏应用 + iOS/Android [节点](https://docs.molt.bot/nodes)。
- **[引导](https://docs.molt.bot/start/wizard) + [技能](https://docs.molt.bot/tools/skills)** — 向导驱动的设置，包含捆绑/管理/工作区技能。
...
### 核心平台
- [网关 WS 控制平面](https://docs.molt.bot/gateway)，包含会话、在线状态、配置、定时任务、Webhook、[控制 UI](https://docs.molt.bot/web) 和[画布主机](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui)。
- [CLI 界面](https://docs.molt.bot/tools/agent-send)：网关、智能体、发送、[向导](https://docs.molt.bot/start/wizard) 和[诊断](https://docs.molt.bot/gateway/doctor)。
- [Pi 智能体运行时](https://docs.molt.bot/concepts/agent) 在 RPC 模式下，支持工具流和块流。
- [会话模型](https://docs.molt.bot/concepts/session)：`main` 用于直接聊天、群组隔离、激活模式、队列模式、回复模式。群组规则：[群组](https://docs.molt.bot/concepts/groups)。
...
- [群组路由](https://docs.molt.bot/concepts/group-messages)：提及门控、回复标签、每个频道的分块和路由。频道规则：[频道](https://docs.molt.bot/channels)。
...
- [iOS 节点](https://docs.molt.bot/platforms/ios)：[画布](https://docs.molt.bot/platforms/mac/canvas)、[语音唤醒](https://docs.molt.bot/nodes/voicewake)、[对话模式](https://docs.molt.bot/nodes/talk)、摄像头、屏幕录制、Bonjour 配对。
- [Android 节点](https://docs.molt.bot/platforms/android)：[画布](https://docs.molt.bot/platforms/mac/canvas)、[对话模式](https://docs.molt.bot/nodes/talk)、摄像头、屏幕录制、可选短信。
...
- [节点](https://docs.molt.bot/nodes)：摄像头快照/剪辑、屏幕录制、[location.get](https://docs.molt.bot/nodes/location-command)、通知。
...
- [技能平台](https://docs.molt.bot/tools/skills)：捆绑、管理和工作区技能，包含安装限制 + UI。
...
### 运维 + 打包
- [控制 UI](https://docs.molt.bot/web) + [WebChat](https://docs.molt.bot/web/webchat) 直接从网关提供服务。
- [Tailscale Serve/Funnel](https://docs.molt.bot/gateway/tailscale) 或 [SSH 隧道](https://docs.molt.bot/gateway/remote)，支持令牌/密码身份验证。
- [Nix 模式](https://docs.molt.bot/install/nix) 用于声明式配置；[Docker](https://docs.molt.bot/install/docker) 基于安装。
- [诊断](https://docs.molt.bot/gateway/doctor) 迁移、[日志](https://docs.molt.bot/logging)。
...
- **[网关 WebSocket 网络](https://docs.molt.bot/concepts/architecture)** — 用于客户端、工具和事件的单一 WS 控制平面（加上运维：[网关运行手册](https://docs.molt.bot/gateway)）。
- **[Tailscale 暴露](https://docs.molt.bot/gateway/tailscale)** — Serve/Funnel 用于网关仪表板 + WS（远程访问：[远程](https://docs.molt.bot/gateway/remote)）。
- **[浏览器控制](https://docs.molt.bot/tools/browser)** — moltbot 管理的 Chrome/Chromium，带有 CDP 控制。
- **[画布 + A2UI](https://docs.molt.bot/platforms/mac/canvas)** — 智能体驱动的可视化工作区（A2UI 主机：[画布/A2UI](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui)）。
- **[语音唤醒](https://docs.molt.bot/nodes/voicewake) + [对话模式](https://docs.molt.bot/nodes/talk)** — 始终在线的语音和持续对话。
- **[节点](https://docs.molt.bot/nodes)** — 画布、摄像头快照/剪辑、屏幕录制、`location.get`、通知，加上 macOS 独有的 `system.run`/`system.notify`。

## Tailscale 访问（网关仪表板）

Moltbot 可以在网关保持绑定到环回地址的同时自动配置 Tailscale **Serve**（仅 tailnet）或 **Funnel**（公共）。配置 `gateway.tailscale.mode`：

- `off`：无 Tailscale 自动化（默认）。
- `serve`：通过 `tailscale serve` 的仅 tailnet HTTPS（默认使用 Tailscale 身份标头）。
- `funnel`：通过 `tailscale funnel` 的公共 HTTPS（需要共享密码身份验证）。
...
在小型 Linux 实例上运行网关是完全正常的。客户端（macOS 应用、CLI、WebChat）可以通过 **Tailscale Serve/Funnel** 或 **SSH 隧道** 连接，您仍然可以配对设备节点（macOS/iOS/Android）以在需要时执行设备本地操作。
...
macOS 应用可以运行在**节点模式**，并通过网关 WebSocket（`node.list` / `node.describe`）通告其功能 + 权限映射。客户端然后可以通过 `node.invoke` 执行本地操作：

- `system.run` 运行本地命令并返回 stdout/stderr/退出代码；设置 `needsScreenRecording: true` 以要求屏幕录制权限（否则您会得到 `PERMISSION_MISSING`）。
...
- 使用这些来协调多个会话之间的工作，而无需在聊天界面之间跳转。
- `sessions_list` — 发现活动会话（智能体）及其元数据。
- `sessions_history` — 获取会话的对话日志。
- `sessions_send` — 向另一个会话发送消息；可选的回复往返 + 公告步骤（`REPLY_SKIP`、`ANNOUNCE_SSKIP`）。

详情：[会话工具](https://docs.molt.bot/concepts/session-tool)

## 技能注册表（ClawdHub）

ClawdHub 是一个最小的技能注册表。启用 ClawdHub 后，智能体可以自动搜索技能并在需要时拉取新的技能。
...
注意：签名构建是 macOS 权限在重建时保持不变所必需的（请参阅 `docs/mac/permissions.md`）。
...
- **默认：** 工具在**主**会话的主机上运行，所以当只有您一个人时，智能体拥有完全访问权限。
- **群组/频道安全：** 设置 `agents.defaults.sandbox.mode: "non-main"` 以在每个会话的 Docker 沙箱内运行**非主会话**（群组/频道）；然后 bash 在这些会话中在 Docker 中运行。
- **沙箱默认：** 允许列表 `bash`、`process`、`read`、`write`、`edit`、`sessions_list`、`sessions_history`、`sessions_send`、`sessions_spawn`；拒绝列表 `browser`、`canvas`、`nodes`、`cron`、`discord`、`gateway`。
...
- 可选：设置 `commands.native`、`commands.text` 或 `commands.useAccessGroups`，加上根据需要的 `channels.discord.dm.allowFrom`、`channels.discord.guilds` 或 `channels.discord.mediaMaxMb`。
...
- 通过 `msteams.allowFrom` 允许谁可以交谈；群组访问通过 `msteams.groupAllowFrom` 或 `msteams.groupPolicy: "open"`。
...
当您完成引导流程并想要更深入的参考时，请使用这些。
- [从文档索引开始导航和"什么在哪里"。](https://docs.molt.bot)
- [阅读架构概览了解网关 + 协议模型。](https://docs.molt.bot/concepts/architecture)
- [当您需要每个键和示例时，使用完整配置参考。](https://docs.molt.bot/gateway/configuration)
- [按照手册规范运行网关。](https://docs.molt.bot/gateway)
- [了解控制 UI/Web 界面如何工作以及如何安全地暴露它们。](https://docs.molt.bot/web)
- [了解通过 SSH 隧道或 tailnet 的远程访问。](https://docs.molt.bot/gateway/remote)
- [按照引导向导流程进行指导设置。](https://docs.molt.bot/start/wizard)
...
- [通过故障排除指南调试常见故障。](https://docs.molt.bot/channels/troubleshooting)
...
- [远程网关自述文件](https://docs.molt.bot/gateway/remote-gateway-readme)
...
- [智能体循环](https://docs.molt.bot/concepts/agent-loop)
...
Moltbot 是为 **Molty** 构建的，这是一个空间龙虾 AI 助手。 🦞
...
请参阅 [CONTRIBUTING.md](CONTRIBUTING.md) 了解准则、维护者以及如何提交 PR。
欢迎 AI/氛围编码的 PR！ 🤖
